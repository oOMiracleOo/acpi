	...
	$( document ).ready( function() {
		...
		ET_PageBuilder.AppView = window.wp.Backbone.View.extend( {
			...
			makeSectionsSortable : function() {
				this.$el.sortable( {
					items  : '> *:not(#et_pb_layout_controls)',
					cancel : '.et-pb-settings, .et-pb-clone, .et-pb-remove, .et-pb-section-add, .et-pb-row-add, .et-pb-insert-module, .et-pb-insert-column',
					update : function( event, ui ) {
						ET_PageBuilder_Events.trigger( 'et-sortable:update' );
					}
				} );
		...
		function et_pb_init_main_settings( $container ) {
			...
			$custom_margin_fields.change( function() {
				...
				$container.find( '.et_custom_margin' ).each( function() {
					margin += $.trim( et_pb_sanitize_input_unit_value( $(this).val() ) ) + '|';
				} );
		...
		function et_pb_sanitize_input_unit_value( value ) {
			...
			if ( this_field_value !== '' ) {
				...
				$margin_fields.each( function() {
					var $this_field = $(this),
						...
						corner_value = et_pb_sanitize_input_unit_value( margins[ field_index ] );
